...
router.get("/delete", function (req, res, next) {
  ...
  // order.findOneAndRemove({'_id' : req.query._id},(err,data)=>{
  //    if(err) {
  //      console.log(err)
  //    }
  //    console.log(data)
...
//   order.findOneAndDelete({'_id' : req.query._id},(err,data)=>{
//     if(err) {
//       console.log(err)
//     }
//     console.log(data)
...
router.get("/sort3", (req, res, next) => {
  const obj = req.query;
    order.find({}).sort({price:3}).exec((err,data)=>{
     if(err){
       console.log(err)
     }
     console.log(data)
...
router.get("/search", (req, res, next) => {
  ...
  order.find({orderName:reg},(err,data)=>{
    if(err){
      console.log(err)
    }
    console.log(data)
